...
export default class Intro extends PureComponent {
  ...
  _tl($elements) {
    ...
    for (let i = 0; i < $elements.length; i++) {
      ...
      const tl = gsap
        ...
        .from(
          splitText.words,
          { duration: 3, opacity: 0, y: 3, ease: "power2", stagger: 0.1 },
          "words"
        )
        .to(splitText.words, {
          duration: 0.2,
  ...
  render() {
    return (
      <Container>
        <Text ref={this.$text1}>
          I en verden hvor <strong>kampen om vår oppmerksomhet</strong> bare
          blir større.
        </Text>
        <Text ref={this.$text2}>
          Så tar vi oss ikke tid til <strong>å faktisk lytte til musikk</strong>
          .
        </Text>
        <Text ref={this.$text3}>
          <strong>{appConfig.ARTIST_NAME}</strong> vil at du skal få en
          fullverdig opplevelse av den nye sangen{" "}
          <strong>{appConfig.SONG_TITLE}</strong>.
        </Text>
        <Text ref={this.$text4}>
          Men det er <strong>en hake</strong>...
        </Text>
        <Text ref={this.$text5}>
          Du må <strong>lukke øynene</strong> for å kunne lytte til sangen.
        </Text>
      </Container>
    );
  }
