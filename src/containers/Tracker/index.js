...
class Tracker extends PureComponent {
  constructor(props) {
    ...
    const gpsParam = urlParams.has("gps")
      ? urlParams.get("gps")
      : "60.3931083,5.3229279"; // Torgallmenningen i Bergen: 60.3931083,5.3229279
    ...
    this.state = {
      ...
      height: 240,
      threshold: 25,
      licenseName: "625-024-049-732-597-692-862-465-335-562-864.vlc",
      licenseURL: "lib/625-024-049-732-597-692-862-465-335-562-864.vlc",
      eyesClosedSession: "",
      eyesClosedCount: 0,
      loadingGeo: false,
      distance: "0 km",
      location: {
        ...
        // latitude: 59.2094395,
        // longitude: 9.607377
        name: urlParams.has("lokasjon")
          ? urlParams.get("lokasjon")
          : "Torgallmenningen i Bergen",
        latitude: gps[0],
        longitude: gps[1],
        distanceThreshold: urlParams.has("distanse")
          ? urlParams.get("distanse")
          : 1,
      },
      geoOptions: {
        maximumAge: 0,
  ...
  componentDidMount() {
    ...
    var preloadFiles = () => {
      ...
      this.visageModule.FS_createPreloadedFile(
        "/",
        "FFT-High.cfg",
        "lib/Facial Features Tracker - High.cfg",
        true,
        false
      );
      ...
      this.visageModule.FS_createPreloadedFile(
        ...
        this.state.licenseURL,
        true,
        false,
        function () {},
        function () {
          console.error("Loading License Failed!");
        }
      );
    };
  ...
  _initGeo() {
    this.setState({ loadingGeo: true }, () =>
      navigator.geolocation.getCurrentPosition(
        this._geolocationSuccess,
        this._geolocationError,
        this.state.geoOptions
      )
    );
  }
  ...
  _geolocationSuccess(position) {
    ...
    console.log(
      `_geolocationSuccess longitude: ${latitude} | latitude: ${longitude}`
    );
  ...
  _watchGeoSuccess(position) {
    const { latitude, longitude } = position.coords;
    console.log(
      `_watchGeoSuccess longitude: ${latitude} | latitude: ${longitude}`
    );
    console.log({ position });
  ...
  _checkLocation(currentLatitude, currentLongitude) {
    ...
    console.log(
      ...
      distanceOutput,
      distanceKm,
      this.state.location.distanceThreshold
    );
  ...
  _readyToCloseEyes() {
    ...
    this.closeEyesTimeline = gsap
      ...
      .from(
        splitText.words,
        { duration: 2, opacity: 0, y: 3, ease: "power2", stagger: 0.1 },
        "words"
      );
  ...
  _loaded() {
    ...
    this.pixels = new Uint8ClampedArray(
      this.visageModule.HEAPU8.buffer,
      this.ppixels,
      this.mWidth * this.mHeight * 4
    );
  ...
  _tick() {
    ...
    this.trackerReturnState = this.m_Tracker.track(
      ...
      this.ppixels,
      this.faceDataArray,
      this.visageModule.VisageTrackerImageFormat.VISAGE_FRAMEGRABBER_FMT_RGBA
        .value,
    ...
    if (
      this.trackerReturnState[0] ===
      this.visageModule.VisageTrackerStatus.TRACK_STAT_OK.value
    ) {
    ...
    if (!this.block) {
      if (this.eyesClosed) {
        ...
        this.music._fadeOut();
        if (this.closeEyesTimeline && this.closedForTheFirstTime)
          this.closeEyesTimeline.play();
      }
    }
        ...
        <Content>
          {!this.state.sdkLoaded && (
            <Loading ref={this.$loader}>
              Laster en ny type opplevelse...
            </Loading>
          )}
          <Intro
            sdkLoaded={this.state.sdkLoaded}
            onComplete={this._introCompleted}
            locationName={this.state.location.name}
          />

          <GeoLocationAccess ref={this.$locationAccess}>
            {this.state.loadingGeo ? (
              <Loading />
            ) : (
              <Fragment>
                <Heading rank={2} asRank={4}>
                  Besøk {this.state.location.name}
                </Heading>
          ...
          <Location ref={this.$location}>
            ...
            <P>
              Du er {this.state.distance} unna. Besøk {this.state.location.name}
              .
            </P>
          </Location>
          ...
          <CameraAccess ref={this.$ready}>
            ...
            <P>
              Vi må ha tilgang til kameraet ditt for å se om du lukker øynene.
            </P>
          ...
          <Completed ref={this.$completed}>
            <Heading rank={2} asRank={4}>
              Vi håper du har fått en ny type opplevelse av{" "}
              {appConfig.SONG_TITLE} fra {appConfig.ARTIST_NAME}.
            </Heading>
            <P>
              Du åpnet øynene {this.state.eyesClosedCount} ganger og din lengste
              sammenhengende økt var {this.state.eyesClosedSession}{" "}
            </P>
          </Completed>

          <Text ref={this.$closeYourEyes}>
            <strong>Lukk øynene</strong> for å lytte.
          </Text>
        </Content>
        <Canvas
          ref={this.$canvas}
